...
export const useGetVegetable = () => {
    return useQuery({
        queryKey: ['objects', 'vegetable'],
        queryFn: async () => {
            const {
                data: listData,
                error: listError,
            } = await supabase.storage.from('menu').list('vegetable')
            ...
            const fileNames = listData?.map((file) => `vegetable/${file.name}`) ?? []
